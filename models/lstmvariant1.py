...
#Load and Train-Test Split Data: When testing the 3 datasets, merely edit which CVS is being read 
...
#LSTM Model Variant 1 - One Directional 
class LSTMOne(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim=64, num_layers=1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.lstm = nn.LSTM(input_size=embed_dim, hidden_size=hidden_dim,num_layers=num_layers, batch_first=True, bidirectional=False)
...
#Initialize Model 
model = LSTMOne(len(vocab), embed_dim).to(device)
...
df.to_csv("datasets/lstm1_dataset1.csv", index=False)
